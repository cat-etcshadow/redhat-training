...
# 95/100 -> WARNING, exit 1
out=$("$SCRIPT_PATH" 95 100) && rc=0 || rc=$?
[[ $rc -eq 1 ]] || fail "95/100 exited $rc, expected 1"
echo "$out" | grep -q "WARNING"    || fail "95/100: missing WARNING in output: $out"

# boundary: 80/100 -> WARNING
out=$("$SCRIPT_PATH" 80 100) && rc=0 || rc=$?
[[ $rc -eq 1 ]] || fail "80/100 (boundary) exited $rc, expected 1 (WARNING)"
