...
for _mp in /mnt/fixme /mnt/backup /mnt/data /mnt/app /mnt/storage \
           /mnt/legacydata /mnt/migratedata /mnt/olddata "${MOUNT_POINT:-}"; do
  [[ -z "$_mp" ]] && continue
...
lvcreate -L 400M -n "$OLD_LV_NAME" "$VG_NAME" &>/dev/null
mkfs.xfs "/dev/${VG_NAME}/${OLD_LV_NAME}" &>/dev/null
mkdir -p "$MOUNT_POINT"

# Deliberately reference the LV by device path (not UUID) — renaming the LV
# changes this path, so the fstab entry breaks unless fixed.
echo "/dev/${VG_NAME}/${OLD_LV_NAME}  $MOUNT_POINT  xfs  defaults  0 0" >> /etc/fstab
mount -a
echo "important data" > "${MOUNT_POINT}/data.txt"
