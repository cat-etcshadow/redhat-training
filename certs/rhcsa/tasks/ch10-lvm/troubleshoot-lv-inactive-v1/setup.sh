...
for _mp in /mnt/fixme /mnt/backup /mnt/data /mnt/app /mnt/storage \
           /mnt/dbdata /mnt/dbstore /mnt/appdata /mnt/vol1 /mnt/vol2 "${MOUNT_POINT:-}"; do
  [[ -z "$_mp" ]] && continue
...
vgcreate rhtr_dbvg "$DISK" &>/dev/null
lvcreate -L 1G -n dbdata rhtr_dbvg &>/dev/null
mkfs.xfs -f /dev/rhtr_dbvg/dbdata &>/dev/null
mkdir -p /mnt/dbdata
UUID=$(blkid -s UUID -o value /dev/rhtr_dbvg/dbdata)
sed -i '/\/mnt\/dbdata/d' /etc/fstab
echo "UUID=$UUID  /mnt/dbdata  xfs  defaults  0 0" >> /etc/fstab

# simulate a maintenance window that left the volume group inactive
umount -f /mnt/dbdata 2>/dev/null || true
vgchange -an rhtr_dbvg &>/dev/null
